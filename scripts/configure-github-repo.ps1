...
.PARAMETER Repo
    GitHub repository name. Defaults to 'ErikLieben.FA.Results'.
...
param(
    ...
    [string]$Repo = "ErikLieben.FA.Results",
    [string]$SecretsFile,
...
# ------------------------------------------------------------------
# 1. Create 'elevation-approved' environment (no reviewers required)
# ------------------------------------------------------------------
Write-Host "`n--- Creating 'elevation-approved' environment ---" -ForegroundColor Yellow

# Create/update the environment without reviewer requirements
# The environment gates secret access for fork PRs via GitHub's built-in behavior;
# no manual approval is needed for the repo owner's own PRs.
$envPayload = @{
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "  [OK] Environment 'elevation-approved' created (no reviewer required)" -ForegroundColor Green
}
else {
